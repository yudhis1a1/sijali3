...
<head>
	...
	<title>TAMPIL RESUME |  LLDikti Wilayah III</title>
	...
	<div class="tile">
		...
		<table width="100%" border="1">
		  ...
		  <tr>
			...
			<td colspan="8">&nbsp;<?=$g_depan?>. <?=$row_dos['nama']?>, <?=$row_dos['gelar_belakang']?></td>
			<?php } else { ?> 
		  ...
		  <tr>
			...
			{
				$tgl_lulus=gfDateStandart($r_pend['tgl_lulus']);
				$tgl_ijazah=$tgl_lulus;
			?>
			&nbsp;<?=$no?>. <?=$r_pend['nama_jenjang']?>, <?=$r_pend['nama_bidang']?> - <?=$r_pend['nm_sp_formal']?>, <?=$tgl_lulus;?><br>
			<?php
			$no++;
			}?>
			</td>
		  ...
		  <tr>
			...
			$kum_baru_d_33=0;
			$jumlah_d_33=0;
			$sql_d_33="select *from usulan_dupaks where usulan_no='$row_dos[no_usulan]' and 
						dupak_no in ('D0033','D0034','D0035') ";
			$data_d_33=mysqli_query($koneksi,$sql_d_33);
			while($data_bid_d_33=mysqli_fetch_array($data_d_33))
			{
				$kum_lama_d_33+=$data_bid_d_33['kum_usulan_lama'];
				$kum_baru_d_33+=$data_bid_d_33['kum_usulan_baru'];
				$jumlah_d_33+=$kum_lama_d_33+$kum_baru_d_33;
			}
			
			if($kum_lama_d_33==0)
			{$da_bid_d_33_lama='';}else{$da_bid_d_33_lama=$kum_lama_d_33;}
			
			if($kum_baru_d_33==0)
			{$da_bid_d_33_baru='';}else{$da_bid_d_33_baru=$kum_baru_d_33;}
			?>
			<td align="center"><?=$da_bid_d_33_lama?></td>
			<td align="center"><?=$da_bid_d_33_baru?></td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<?php
			$jml_baru_11_ok=0;
