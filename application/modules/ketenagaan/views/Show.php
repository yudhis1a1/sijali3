...
<div class="row page-titles">
	...
	<div class="form-group m-t-40 row">
		...
		<div class="col-md-12">
			<?php $catatan = $this->db->query("SELECT * from usulan_riwayat_statuses where usulan_no='$nox' AND keterangan IS NOT NULL  ORDER BY updated_at DESC ")->result_array(); ?>
			<label for="ket">Keterangan Internal</label>
			<textarea name="ket" class="form-control" rows="4" readonly> <?php foreach ($catatan as $cat_ptk) : echo $cat_ptk['keterangan'] . '; ';
																			endforeach; ?> </textarea>
		</div>
	</div>
</div>
...
<div class="card-body">
	<?php
	if ($data_dasar->usulan_status_id == '3' || $data_dasar->usulan_status_id == '8' || $data_dasar->usulan_status_id == '12' || $data_dasar->usulan_status_id == '13' || $data_dasar->usulan_status_id == '14' || $data_dasar->usulan_status_id == '15' || $data_dasar->usulan_status_id == '6' || $data_dasar->usulan_status_id == '7' || $data_dasar->usulan_status_id == '9' || $data_dasar->usulan_status_id == '11') {
	?>
	...
	<?php
	if ($data_dasar->usulan_status_id == '12') {
		$status_id_new = "pending_nilai";
	} else {
		$status_id_new = $status_id;
	}
	?>
	<a href="<?php echo base_url('ketenagaan/ketenagaan/' . $status_id_new); ?>" class="btn btn-success">Kembali</a>
	<?php if ($data_dasar->usulan_status_id == '13'  || $data_dasar->usulan_status_id == '6' || $data_dasar->usulan_status_id == '12') {
	?>
	...
	<?php } ?>
	<?php if ($data_dasar->usulan_status_id == '3'  && empty($data_dasar->pic_validasi) && empty($data_dasar->user_penilai_no)) {
	?>
		<a href="#validasiModal" data-toggle="modal" class="btn btn-rounded btn-outline-danger fa fa-check-square-o pull-right" id="btn_validasi"> Validasi</a>
	<?php }
	if ($data_dasar->usulan_status_id == '11') {
	?>

		<a href="#uploadGbModal" data-toggle="modal" class="btn btn-info">
...
<div class="row">
	<div class="col-md-12">
		<div class="card">
			<div class="card-body">
				...
				<ul class="nav nav-tabs" role="tablist">
					...
					<?php if ($usul_jab->kode >= '46') { ?>
						<li class="nav-item"> <a class="nav-link" data-toggle="tab" id="nav_bid_e" href="#bidang_E" role="tab"><span class="hidden-sm-up"><i class="ti-agenda "></i></span> <span class="hidden-xs-down">Tambahan LK/GB</span></a> </li>
					<?php } ?>
					...
					<?php if (($usulan_status_id == '6' && $v_usulans->user_penilai_keputusan == 'DITERIMA') || ($usulan_status_id == '14') || ($usulan_status_id == '15') || ($usulan_status_id == '8')) {  ?>
					...
					<?php if ($usulan_status_id == '9') { ?>
						<li class="nav-item"> <a class="nav-link" data-toggle="tab" id="nav_doc" href="#doc" role="tab"><span class="hidden-sm-up"><i class="ti-bookmark-alt "></i></span> <span class="hidden-xs-down">File PAK Dan SK</span></a> </li>
					<?php } ?>
					<?php if ($usulan_status_id == '15' || $usulan_status_id == '8') { ?>
						<li class="nav-item"> <a class="nav-link" data-toggle="tab" id="nav_sk" href="#sk" role="tab"><span class="hidden-sm-up"><i class="ti-ruler-pencil "></i></span> <span class="hidden-xs-down">SK</span></a> </li>
					<?php } ?>
				</ul>
				<!-- Tab panes -->
		...
		WHERE a.`nidn` = '$data_dasar->nidn'")->row();
									...
									?>
										...
										<?php
										$r_tmt = $this->db->query("SELECT
			`tmt_tahun`,
			`tmt_bulan`
			FROM
			tmt_jab
			WHERE `usulan_no` = '$data_dasar->no'
			AND `dosen_no` = '$data_dasar->dosen_no'")->row();
					...
					<div class="tab-pane p-20" id="bidang_E" role="tabpanel"></div>
					...
					<div class="tab-pane  p-20" id="doc" role="tabpanel"></div>
					<div class="tab-pane  p-20" id="sk" role="tabpanel"></div>
...
<script>
	...
	$(function() {
		$urlbidang_e = "<?php echo base_url('ketenagaan/ketenagaan/bidangE/' . $no_usulan); ?>";
		$('#nav_bid_e').click(function() {
			$('#bidang_E').load($urlbidang_e);
			$(this).removeClass('active');

		});
	})
	$(function() {
		$urldoc = "<?php echo base_url('ketenagaan/ketenagaan/show_vpak/' . $no_usulan); ?>";
	...
	$(function() {
		$urlsk = "<?php echo base_url('kepegawaian/kepegawaian/show_pak/' . $no_usulan); ?>";
		$('#nav_sk').click(function() {
			$('#sk').load($urlsk);
			$(this).removeClass('active');
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<input type="hidden" class="form-control" value="<?php echo $status_id; ?>" name="statusnya">
						<input type="hidden" class="form-control" value="<?php echo $no_usulan; ?>" name="no_usulan">

					</div>

					<div class="form-group">
						<label for="penilai">Tim Penilai </label>
						<input type="hidden" class="form-control" value="<?php echo $data_dasar->user_penilai_no; ?>" name="id_penilai">
					...
					<div class="form-group">
						...
						<select name="status_usulan" class="form-control select" id="status_usulan" data-placeholder="Click to Choose..." required>
							<option value=""></option>
							<?php
							if ($data_dasar->usulan_status_id == '3') {
							?>
								<option <?php if ($data_dasar->usulan_status_id == '13') echo 'selected'; ?> value="13">Validasi Persyaratan Administrasi</option>
								<option <?php if ($data_dasar->usulan_status_id == '2') echo 'selected'; ?> value="2">Proses Revisi Usulan</option>

							<?php }
							if ($data_dasar->usulan_status_id == '4' || $data_dasar->usulan_status_id == '6' || $data_dasar->usulan_status_id == '7') {
							?>
								<option <?php if ($data_dasar->usulan_status_id == '2') echo 'selected'; ?> value="2">Proses Revisi Usulan</option>
							<?php
							}
							if ($data_dasar->usulan_status_id == '13' || $data_dasar->usulan_status_id == '4' || $data_dasar->usulan_status_id == '12') { ?>
								<option <?php if ($data_dasar->usulan_status_id == '5') echo 'selected'; ?> value="5">Proses Tim Penilai</option>
								<option <?php if ($data_dasar->usulan_status_id == '3') echo 'selected'; ?> value="3">Kembali Ke Usulan Baru</option>
							<?php
							}
							if ($data_dasar->usulan_status_id == '7') {
							?>
								<option <?php if ($data_dasar->usulan_status_id == '11') echo 'selected'; ?> value="11">Diusulkan Ke Dikti</option>
								<option <?php if ($data_dasar->usulan_status_id == '5') echo 'selected'; ?> value="5">Proses Tim Penilai</option>

								<option <?php if ($data_dasar->usulan_status_id == '14') echo 'selected'; ?> value="14">Proses Validasi PAK & SK</option>
							<?php
							}
							if ($data_dasar->usulan_status_id == '6') { ?>
							...
							if ($data_dasar->usulan_status_id == '9') { ?>
								<!-- <option <?php if ($data_dasar->usulan_status_id == '3') echo 'selected'; ?> value="3">Kembali Ke Usulan Baru</option> -->
								<option <?php if ($data_dasar->usulan_status_id == '8') echo 'selected'; ?> value="8">Kembali Ke Operator Sub HKT</option>
								<option <?php if ($data_dasar->usulan_status_id == '15') echo 'selected'; ?> value="15">Ralat PAK</option>
							<?php

							}

							if ($data_dasar->usulan_status_id == '14') { ?>

								<option <?php if ($data_dasar->usulan_status_id == '2') echo 'selected'; ?> value="2">Proses Revisi Usulan</option>
								<option <?php if ($data_dasar->usulan_status_id == '6') echo 'selected'; ?> value="6">Proses Operator Sub PTK</option>
								<option <?php if ($data_dasar->usulan_status_id == '8') echo 'selected'; ?> value="8">Proses Operator Sub HKT</option>

							<?php
							}
							if ($data_dasar->usulan_status_id == '15') { ?>

								<option <?php if ($data_dasar->usulan_status_id == '8') echo 'selected'; ?> value="8">Proses Operator Sub HKT</option>
								<option <?php if ($data_dasar->usulan_status_id == '9') echo 'selected'; ?> value="9">Selesai</option>
							<?php
							}
							if ($data_dasar->usulan_status_id == '11') { ?>
							...
							if ($data_dasar->usulan_status_id == '8') { ?>
					...
					?>
						<div class="form-group">
							...
							<textarea name="keterangan_pengusul" class="form-control" id="keterangan_pengusul" rows="4"><?php foreach ($catatan as $cat_ptk) : echo $cat_ptk['keterangan'] . '; ';
																														endforeach; ?></textarea>
						</div>
					<?php  } else { ?>
					...
					<div class="form-group">
						<label for="berkas">Lampiran</label>
						<input type="file" name="berkas" class="form-control" id="berkas" accept="application/pdf,.doc, .docx, .png, .jpg, .jpeg" max="5048">
						<p class="help-block">Ukuran file maksimal 5MB. Ekstensi .pdf .doc .docx, .jpg, .jpeg, .png</p>
					</div>
...
<form id="addPenilaiForm" method="post" action="<?= base_url() ?>ketenagaan/ketenagaan/updatePenilai/<?php echo $no_usulan; ?>" role="form" enctype="multipart/form-data" onsubmit="return cekSubmitAddPenilai()">
	...
	<div class="modal fade" id="addPenilaiModal" role="dialog" aria-labelledby="addPenilaiModalLabel" style="overflow:hidden;">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" value="<?php echo $penilai->nama . ' (' . $penilai->nama_instansi . ')'; ?>" readonly>
						<input type="hidden" class="form-control" value="<?php echo $no_usulan; ?>" name="no_usulan">
						<input type="hidden" name="usulan_status_id" value="<?php echo $data_dasar->usulan_status_id; ?>">
					</div>
...
<!-- validasiL -->
<form id="validasi_data" method="post" action="<?= base_url() ?>ketenagaan/ketenagaan/validasi_data/<?php echo $no_usulan; ?>" role="form" enctype="multipart/form-data">

	<div class="modal fade" id="validasiModal" role="dialog" aria-labelledby="validasi_dataModalLabel" style="overflow:hidden;">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
...
<form method="post" action="<?= base_url() ?>ketenagaan/ketenagaan/uploadGb" role="form" enctype="multipart/form-data">
	<div class="modal fade" id="uploadGbModal" role="dialog" aria-labelledby="uploadGbModal">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<h4>Upload FILE Dokumen SK LK/GB</h4>
