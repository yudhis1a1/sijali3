...
class Ketenagaan extends MX_Controller
{
	...
	public function tampil_data_dosen()
	{
		...
		foreach ($hasil as $h) {
			$no 				= $h['id'];
			$nidn 				= $h['nidn'];
			$data['nama']		= addslashes($h['nama']);
			$data['prodi_kode']	= $h['id_prodi'];
			$data['kode_prodi']	= $h['kode_prodi'];
			$data['nm_prodi']	= $h['nm_prodi'];
			$data['jk']			= $h['jenis_kelamin'];
			$data['kode_pt']	= $h['kode_pt'];
			$data['nm_pt']		= $h['nm_pt'];
			$data['tmt_sk_pengangkatan'] = $h['tmt_sk_pengangkatan'];

			$ikatan_kerja_kode 	= $h['ikatan_kerja']['id'];
			$golongan_kode 		= $h['pangkat_golongan']['id'];
			$jabatan_no 		= $h['jabatan_fungsional']['id'];
			...
			$sql3 = $this->db->query("SELECT * FROM golongans WHERE kode='$golongan_kode'")->row();

			//cari Jabatans
			$sql4 = $this->db->query("SELECT * FROM jabatans WHERE kode='$jabatan_no'")->row();

			//cari Ikatan Kerja
			$sql5 = $this->db->query("SELECT * FROM ikatan_kerjas WHERE kode='$ikatan_kerja_kode'")->row();
		}
	...
	function update_data_dosen()
	{
		...
		foreach ($hasil as $h) {
			...
			$up = $this->db->query("UPDATE `dosens` SET `prodi_kode`='$prodi_kode' WHERE `nidn`='$nidn_dos'");
		}
	...
	function usulanbaru()
	{
		...
		$data['staf'] = $this->db->query("SELECT * FROM `users` a WHERE a.`role_id`='6' AND a.`id_parent`='29'  ORDER BY a.`nama` ASC")->result();
		Vusulan('Beranda', $data);
		$this->load->view('modalptk');
	}
	...
	function statusUsulan($sts_usulan)
	{
		...
		if ($sts_usulan == '3') {
			...
			$status = 'ralat_pak';
		} else if ($sts_usulan == '11') {
			$status = 'proses_ke_dikti';
		}
		return $status;
	}
	...
	function hapus_pendidikan_pak($id_rwy_didik, $no_usulan)
	{
		...
		$log = "INSERT INTO usulan_riwayat_statuses(no,usulan_no,usulan_status_id,user_no,keterangan,created_at,updated_at)
			values ('$no_log','$no_usulan','6','$userno','$keterangan',now(),now())";
		$this->db->query($log);
	...
	function Show_pendidikan($no_usulan)
	{
		...
		$sinkron_pen	= $this->db->query("SELECT sinkron_at from rwy_pend_formal WHERE id_sdm='$r_nidn->dosen_no' GROUP by id_sdm")->row();
	...
	function bidangA($no_usulan)
	{
		...
		$showa['no'] = $no_usulan;
		$showa['qr_dupak'] = $this->db->query("SELECT * FROM usulan_dupaks a WHERE  a.usulan_no='$no_usulan'")->result();
		$this->load->view('Show_a', $showa);
	}
	function bidang_A($no_usulan)
	{
	...
	function bidangB($no_usulan)
	{
		...
		$data = $this->db->query("SELECT * from usulans where no='$no_usulan'")->row();
		$usulan_status_id = $data->usulan_status_id;
		$showb['role'] = $this->session->userdata('role');
		$showb['usulan_status_id'] = $usulan_status_id;
		$showb['no'] = $no_usulan;
		$showb['qrb_dupak'] = $this->db->query("SELECT * FROM usulan_dupaks a WHERE  a.usulan_no='$no_usulan'")->result();
	...
	function bidangC($no_usulan)
	{
		...
		$data = $this->db->query("SELECT * from usulans where no='$no_usulan'")->row();
		$usulan_status_id = $data->usulan_status_id;
		$showc['role'] = $this->session->userdata('role');
		$showc['usulan_status_id'] = $usulan_status_id;
		$showc['qrc_dupak'] = $this->db->query("SELECT * FROM usulan_dupaks a WHERE  a.usulan_no='$no_usulan'")->result();
		$showc['no'] = $no_usulan;
	...
	function bidangD($no_usulan)
	{
		...
		$showd['role'] = $this->session->userdata('role');
		$data = $this->db->query("SELECT * from usulans where no='$no_usulan'")->row();
		$usulan_status_id = $data->usulan_status_id;
		$showd['qrd_dupak'] = $this->db->query("SELECT * FROM usulan_dupaks a WHERE  a.usulan_no='$no_usulan'")->result();
	...
	function show_ceklist($no_usulan)
	{
		...
		$show_syarat['role'] 				= $role;
		$show_syarat['jabatan_no'] 		= $data_isi->jabatan_no;
		$show_syarat['jabatan_usulan_no'] = $data_isi->jabatan_usulan_no;
		$show_syarat['pengangkatan_tgl'] 	= $data_isi->pengangkatan_tgl;
		$show_syarat['usulan_status'] 	= $data_isi->usulan_status_id;
		$show_syarat['isi'] 				= $data_isi;
		// utk usulan LK 
		if ($data_isi->jabatan_usulan_no == '8' || $data_isi->jabatan_usulan_no == '9' || $data_isi->jabatan_usulan_no == '10' || $data_isi->jabatan_usulan_no == '11' || $data_isi->jabatan_usulan_no == '12' || $data_isi->jabatan_usulan_no == '13' || $data_isi->jabatan_usulan_no == '14' || $data_isi->jabatan_usulan_no == '15') {
			$this->load->view('Show_persyaratan_lk_gb', $show_syarat);
		} else {
			$this->load->view('Show_persyaratan', $show_syarat);
		}
	}

	function validate_ok($no_usulan, $jenis)
	{
		$user = $this->session->userdata('nama');
		...
		$inserdata = $this->ok_cek($data);



		redirect(base_url() . 'ketenagaan/ketenagaan/show/' . $no_usulan);
	}
	function validate_x($no_usulan)
	{
		$user = $this->session->userdata('nama');
		$jenis = $this->input->post('jnsberkas');
	...
	function ok_cek($data)
	{
		$this->db->replace('rwy_ceklist', $data);
	...
	function validate_dupax($no_usulan)
	{
		$user = $this->session->userdata('nama');
		$nodupak = substr($this->input->post('nodupak'), 0, 1);
		$keterangan = $this->input->post('keterangan');
		$tgl_log = date("y-m-d H:i:s");
		$data = array(
			'valid' => '400',
		...
		$this->db->update('usulan_dupaks', $data, array('usulan_no' => $no_usulan, 'dupak_no' => $nodupak));
		$logd = "INSERT INTO rwy_dupaks_validasi (
  `usulan_no`,
  `no_dupaks`,
  `status`,
  `keterangan`,
  ...
  `updated_at`
	...
	VALUES
		...
		$this->db->query($logd);
		redirect(base_url() . 'ketenagaan/ketenagaan/bidang_' . $nodupak . '/' . $no_usulan);
	}
	function validate_a($no_usulan, $dupak)
	{
		$user = $this->session->userdata('nama');
		$nodupak = substr($dupak, 0, 1);
		$tgl_log = date("y-m-d H:i:s");
		$data = array(
			'valid' => '200',
			'keterangan' => 'Valid',
		...
		$this->db->update('usulan_dupaks', $data, array('usulan_no' => $no_usulan, 'dupak_no' => $dupak));
		$log_a = "INSERT INTO rwy_dupaks_validasi (
		`usulan_no`,
		`no_dupaks`,
		`status`,
		`keterangan`,
	...
	function pushnilai_kum($no_usulan)
	{
		$user = $this->session->userdata('nama');
		$nodupak = $this->input->post('nodupak');
		$nilaikum = $this->input->post('nilai_kum');
		$tgl_log = date("y-m-d H:i:s");
		$no_log = date("ymdHis");
		$keterangan = 'Kum Penilaian diupdate oleh: ' . $user;
		$datakum = array(
			'kum_penilai_baru' => $nilaikum,
			'user_update_kum' => $user,
			'tgl_update_kum' => $tgl_log
		);
		$this->db->update('usulan_dupaks', $datakum, array('usulan_no' => $no_usulan, 'dupak_no' => $nodupak));
		$log_kum = "INSERT INTO riwayat_kum_dupak (
		`no`,
		`usulan_no`,
		`user_nama`,
		`keterangan`,
	...
	function usulan($filter)
	{
		...
		$cekId = $this->db->query("SELECT * FROM `users` WHERE NO='$userno' ")->row();
		if (($cekId->id_parent == '29') && ($filter == '3')) {
			$this->db->select('*');
			$this->db->from('v_usulans');
			$this->db->where('usulan_status_id', $id_sts);
			$this->db->where('id_user_val', $userno);
		...
		if (($cekId->id_parent == '29') && ($filter == '3')) {
			...
			if ($search != "") {
				...
				$query = $this->db->query("select * from v_usulans where (usulan_status_id='$filter' and no LIKE '%$search%' and id_user_val='$userno') or (usulan_status_id='$filter' and nidn LIKE '%$search%' and id_user_val='$userno') or (usulan_status_id='$filter' and nidk LIKE '%$search%' and id_user_val='$userno') or 
	 (usulan_status_id='$filter' and nama LIKE '%$search%' and id_user_val='$userno') or (usulan_status_id='$filter' and  nama_instansi LIKE '%$search%' and id_user_val='$userno') or  (usulan_status_id='$filter' and nama_prodi LIKE '%$search%'  and id_user_val='$userno')  or  (nama='$filter' and no LIKE '%$search%' and id_user_val='$userno') or
	 (usulan_status_id='$filter' and user_penilai_keputusan LIKE '%$search%'  and id_user_val='$userno')");
	 ...
	 (usulan_status_id='$filter' and user_penilai_keputusan LIKE '%$search%' )");
		...
		foreach ($query->result_array() as $surat) {
			...
			$id_pic = $surat['id_user_val'];
			$nm_pic  = $this->get_userPtk($id_pic);
			if ($filter == '5' || $filter == '6' || $filter == '8') {
				$status_penilaian =	$surat['user_penilai_keputusan'];
			} else {
				$status_penilaian = '-';
			}
			if (($filter == '3') && ($id_pic == '') && ($cekId->id_parent == '7')) {
				$detail = '<a href=show/' . $surat['no'] . ' data-toggle="tooltip" title="Detail Ajuan"><button type="button" class="btn btn-sm btn-circle btn-primary"><i class="  icon-book-open" ></i> </button></a><br/>
			   <a href="javascript:void(0)" data-toggle="modal" data-target="#usulanbaru-' . $surat['no'] . '" class="btn btn-sm btn-circle btn-info"><i class="  icon-people" ></i></a>';
			} else if (($filter == '3') && ($id_pic != '') && ($cekId->id_parent == '7' || ($cekId->id_parent == '29'))) {
				$detail = '<a href=show/' . $surat['no'] . ' data-toggle="tooltip" title="Detail Ajuan"><button type="button" class="btn btn-sm btn-circle btn-primary"><i class="  icon-book-open" ></i> </button></a> <br/>
				<a class="btn btn-circle btn-sm btn-success " data-toggle="collapse" href="#collapseExample-' . $surat['no'] . '" aria-expanded="false" aria-controls="collapseExample"><i class="icon-check" ></i>
				</a><div class="collapse" id="collapseExample-' . $surat['no'] . '"><p class="text-info font-weight-bold">' . $nm_pic . '</p></div>';
			} else {
				$detail = '<a href=show/' . $surat['no'] . ' data-toggle="tooltip" title="Detail Ajuan"><button type="button" class="btn btn-sm btn-circle btn-primary"><i class="  icon-book-open" ></i> </button></a>';
			}
			$output['data'][] = array(
				$detail,
	...
	function updatePak($no_usulan)
	{
		...
		$log = "INSERT INTO usulan_riwayat_statuses(no,usulan_no,usulan_status_id,user_no,keterangan,created_at,updated_at)
			  values ('$no_log','$no_usulan','$usulan_status_id','$user_no','$keterangan',now(),now())";
		$this->db->query($log);
	...
	function revisiPak($no_usulan)
	{
		...
		$log = "INSERT INTO usulan_riwayat_statuses(no,usulan_no,usulan_status_id,user_no,keterangan,created_at,updated_at)
		values ('$no_log','$no_usulan','15','$user_no','$keterangan',now(),now())";
		$this->db->query($log);
	...
	function validasi_data($no_usulan)
	{
		...
		$log = "INSERT INTO usulan_riwayat_statuses(no,usulan_no,usulan_status_id,user_no,keterangan,created_at,updated_at)
				values ('$no_log','$no_usulan','$usulan_status_id','$userno','$keterangan',now(),now())";
		$this->db->query($log);
	...
	function uploadPak()
	{
		...
		if ($data_pak > 0) {
			...
			$log = "INSERT INTO usulan_riwayat_statuses(no,usulan_no,user_no,keterangan,created_at,updated_at)
			values ('$no_log','$no_usulan','$userno','$keterangan',now(),now())";
			$this->db->query($log);
	...
	function uploadGb()
	{
		...
		$tgl_update = date("y-m-d H:i:s");
		$no = date("ymdHis");
		$file_path = './assets/LkGb/';
		mkdir($file_path, 0777, true);
		$config['upload_path'] = $file_path;
		$config['allowed_types'] = 'pdf';
		$config['max_size'] = '5048';
		$config['file_name'] = 'LKGB_' . $no . '_' . $nidn;
		$this->load->library('upload', $config);
		$no_log = date("ymdHis");



		$keterangan = "Data SK LK/GB diupload oleh:" . $user;

		$data_pak = $this->db->query("SELECT no FROM `usulans` WHERE no='$no_usulan'")->num_rows();
		if ($data_pak > 0) {

			if ($this->upload->do_upload('berkas')) {
				$lkgb = $this->upload->data();
				$update = "update usulans set  sk_gb= '$lkgb[file_name]',user_updated_no = '$userno', skgb_updater = '$userno' where  no='$no_usulan'";
				$this->db->query($update);
			} else {
				$this->session->set_flashdata('error', 'Format dan Ukuran File Tidak Sesuai');
				redirect(base_url() . 'ketenagaan/ketenagaan/show/' . $no_usulan);
 ...
 WHERE b.`dupak_no` = a.`no`
  ...
  GROUP BY b.`usulan_no`")->row();
	...
	function kirimUsulan()
	{
		...
		$user = $this->session->userdata('nama');
		$noUsulan = $this->input->post('noUsulan');
		$stafptk = $this->input->post('stafptk');
		//$tgl_log = date("y-m-d H:i:s");
		//$keterangan = "Penugasan Staf oleh :" . $user;
		/* echo $stafptk."<br/>";
echo $noUsulan."<br/>";
exit();
		 */
		$updateStaf = "UPDATE usulans set id_user_val='$stafptk' WHERE  no='$noUsulan'";
		$this->db->query($updateStaf);
		redirect(base_url() . 'ketenagaan/ketenagaan/usulanbaru/');
	}
}
