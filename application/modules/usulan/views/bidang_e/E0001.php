...
<div class="row">
	...
	<div class="card">
	<div class="card-body">
	<a href="" data-toggle="modal" id="myLargeModalLabel" data-target="#tambah"  class="btn btn-success"><i class="fa fa-plus-square"> TAMBAH</i></a><br><br>
		<table border="1" class="ui celled table" width="100%">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<a href="<?= base_url()?>usulan/usulan_dupak_e/hapus_E0001/<?=$row->no?>/<?=$no?>/<?=$row->dupak_no?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>usulan/usulan_dupak_e/tambah_E0001/<?=$dupak_no?>/<?= $no ?>" role="form" enctype="multipart/form-data">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="edit<?=$row->no;?>" >
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
		<form method="post" action="<?= base_url()?>usulan/usulan_dupak_e/update_e0001/<?=$row->dupak_no;?>/<?=$row->no?>/<?=$no?>" role="form" enctype="multipart/form-data">
		<div class="modal-body">
			<h3>Kegiatan Tambahan LK/GB</h3>
			<b>Mendapatkan Hibah Penelitian Kompetitif (sebagai ketua)</b><br>
			<hr>
