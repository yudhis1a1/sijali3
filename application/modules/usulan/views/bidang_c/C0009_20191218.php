...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
			...
			Terjadwal/terprogram Kurang dari satu semester dan minimal satu bulan Insidental
		</div>
    </div>
	</div>
	</div>
		...
		<table border="1" class="ui celled table" width="100%">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<td><?=$row->keterangan?><br><a href="<?= base_url()?>usulan/usulan_dupak_c/download_bidangc/<?=$row->berkas?>" class="btn  btn-success" ><i class="fa  fa-cloud-download"></i>[PDF]</a> <br>
					<?php  if(isset($row->modul)){ ?>
					<a href="<?= base_url()?>assets/usulan_dupak_c/<?= $row->modul ?>" target="_blank" class="btn btn-sm btn-success" >
					<i class="fa  fa-cloud-download"></i>[Modul]</a>
					<?php  }else{ ?>
				  	<a href="#uploadModal" data-toggle="modal" data-nama="modul,<?=$row->no?>" data-judul="Upload Modul" class="btn  btn-sm btn-warning" ><i class="fa fa-upload"></i>[Modul]</a>
					<?php  } ?>
					<?php  if(isset($row->proposal)){ ?>
					<a href="<?= base_url()?>assets/usulan_dupak_c/<?= $row->proposal ?>" target="_blank" class="btn btn-sm btn-success" >
					<i class="fa  fa-cloud-download"></i>[Proposal]</a>
					<?php  }else{ ?>
				  	<a href="#uploadModal" data-toggle="modal" data-nama="proposal,<?=$row->no?>" data-judul="Upload Proposal" class="btn  btn-sm btn-warning" ><i class="fa fa-upload"></i>[Proposal]</a>
					<?php  } ?>
					<?php  if(isset($row->laporan)){ ?>
					<a href="<?= base_url()?>assets/usulan_dupak_c/<?= $row->laporan ?>" target="_blank" class="btn btn-sm btn-success" >
					<i class="fa  fa-cloud-download"></i>[Laporan]</a>
					<?php  }else{ ?>
				  	<a href="#uploadModal" data-toggle="modal" data-nama="laporan,<?=$row->no?>" data-judul="Upload Laporan" class="btn  btn-sm btn-warning" ><i class="fa fa-upload"></i>[Laporan]</a>
					<?php  } ?>
					</td>
					<td>
					...
					<a href="<?= base_url()?>usulan/usulan_dupak_c/hapus_C0001/<?=$row->no?>/<?=$row->usulan_no?>/C0009/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
  		    <a href="<?= base_url()?>usulan/usulan/bidangC/<?=$no?>" class="btn btn-success"><i class="fa fa-backward"></i> Kembali Ke Detail Usulan Dosen</a>
		</div>
    </div>
	</div>
	</div>
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>usulan/usulan_dupak_c/tambah_C0001/C0009/<?= $kum ?>" role="form" enctype="multipart/form-data">
		<div class="modal-body">
				<h3>Bidang C</h3>
				<b>Memberi latihan/penyuluhan/penataran/ceramah pada masyarakat</b><br>
			Terjadwal/terprogram Kurang dari satu semester dan minimal satu bulan Tingkat Lokal
                <hr>
                ...
                <div class="form-group">
				  ...
				  <input type="hidden" name="kum" value="1"required>
                </div>
				</div>
				</div>
				<div class="row">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="edit<?=$row->no;?>" >
	...
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>usulan/usulan_dupak_c/update_c0001/<?=$row->dupak_no;?>" role="form" enctype="multipart/form-data">
		<div class="modal-body">
			<h3>Bidang C</h3>
				<b>Memberi latihan/penyuluhan/penataran/ceramah pada masyarakat</b><br>
			Terjadwal/terprogram Kurang dari satu semester dan minimal satu bulan Tingkat Lokal
...
<!-- Modal Upload -->
<?php foreach($q3->result() as $row){?>
<form  method="post" action="<?= base_url()?>usulan/usulan_dupak_c/upload_file/C0009" role="form" enctype="multipart/form-data">
