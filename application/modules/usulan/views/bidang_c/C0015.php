...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
  		    <h3>BIDANG C</h3>
			<b>Berperan serta aktif dalam pengelolaan jurnal ilmiah (per tahun)*</b><br>
			Editor/dewan penyunting/dewan redaksi jurnal ilmiah internasional
		</div>
    </div>
	</div>
	</div>
	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<tbody>
					...
					<tr class="text-center">
						<?php if($role<>'3' || $usulan_status_id<>'1' && $usulan_status_id<>'2'){echo "<td></td><td></td>";}else{ ?>
						<td>
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<?php  if(isset($row->berkas)){ ?>
						...
						<?php }else{ ?>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/download_bidangc/<?=$row->berkas?>" class="btn btn-xs btn-success" ><i class="fa  fa-cloud-download"></i>[Surat Tugas]</a>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/hapus_berkas/<?=$row->berkas?>/<?= $row->usulan_no ?>/berkas/C0015" class="btn btn-xs btn-danger tombol-hapus"><i class="fa fa-trash"></i></a><br>
						<?php } ?>
					<?php  }else{ ?>
						<?php if($role<>'3'){?>
							<a href="#" class="btn  btn-xs btn-danger" ><i class="fa fa-upload"></i>[Surat Tugas]</a><br>
						<?php }else{ ?>
							<a href="#uploadModal" data-toggle="modal" data-nama="berkas,<?=$row->no?>" data-judul="Upload SK Surat Tugas" class="btn  btn-xs btn-warning" ><i class="fa fa-upload"></i>[Surat Tugas]</a><br>
						<?php } ?>
					<?php  } ?>
					...
					<?php  if(isset($row->sertifikat)){ ?>
						...
						<?php }else{ ?>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/download_bidangc/<?= $row->sertifikat ?>" target="_blank" class="btn btn-xs btn-success" ><i class="fa  fa-cloud-download"></i>[Sertifikat]</a>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/hapus_berkas/<?=$row->sertifikat?>/<?= $row->usulan_no ?>/sertifikat/C0015" class="btn btn-xs btn-danger tombol-hapus">
					...
					<?php  }else{ ?>
						...
						<?php }else{ ?>
							<a href="#uploadModal" data-toggle="modal" data-nama="sertifikat,<?=$row->no?>" data-judul="Upload Sertifikat" class="btn  btn-xs btn-warning" ><i class="fa fa-upload"></i>[Sertifikat]</a>
							<?php  } ?>
					<?php  } ?>
					...
					<a href="<?= base_url()?>usulan/usulan_dupak_c/hapus_C0001/<?=$row->no?>/<?=$row->usulan_no?>/C0015/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>usulan/usulan_dupak_c/tambah_C0015/C0015/<?=$kum?>" role="form" enctype="multipart/form-data">
                ...
                <div class="row">
			      <div class="col-md-4">
                    <div class="form-group">
					  ...
					  <input type="semester" name="semester" value="<?=$row->tahun_akademik?>/<?=$row->thn_akademik_baru?>"  class="form-control" id="semester" maxlength="150" readonly>
					</div>
                  </div>
...
<!-- Modal Upload -->
<?php foreach($q3->result() as $row){?>
<form  method="post" action="<?= base_url()?>usulan/usulan_dupak_c/upload_file/C0015" role="form" enctype="multipart/form-data">
