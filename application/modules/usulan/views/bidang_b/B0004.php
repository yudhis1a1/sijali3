				...
				<div class="card-body">
					...
					<div class="table-responsive m-t-40">
						<form method="post" action="<?= base_url() ?>usulan/usulan_dupak_b/update_peer/<?= $dupak_no ?>" role="form" enctype="multipart/form-data">
							<table border="1" class="ui celled nowarp table" width="100%">
								<thead>
									<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
										<th>Judul</th>
										<th>Semester</th>
									...
									<?php foreach ($q3->result() as $row) { ?>
										<tr class="text-center">
											...
											<td>
												...
												URL Cek Similarity atau Originality : <a href="<?= $row->url_index ?>" target="_blank"><?= $row->url_index ?></a><br>
											...
											<td>
												<?php if ($usulan_status_id <> '1' && $usulan_status_id <> '2') {
													...
													<a href="<?= base_url() ?>usulan/usulan_dupak_b/hapus_B0004/<?= $row->no ?>/<?= $row->usulan_no ?>/<?= $row->berkas ?>/B0004" class="btn btn-danger tombol-hapus">
