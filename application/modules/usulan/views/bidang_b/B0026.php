		...
		<div class="card">
			<div class="card-body">
				<div class="tile">
					...
					Jurnal internasional terindeks pada basis data internasional di luar
				</div>
			</div>
		</div>
	</div>
							...
							<table border="1" class="ui celled table" width="100%">
								<thead>
									<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
										...
										<th>URL</th>
										<th>Keterangan</th>
										<th>Jumlah Penulis/Penulis ke/Koresponden</th>
										<th>Syarat Khusus</th>
										<th>Satuan Hasil</th>
										<th>Angka Kredit</th>
										<th>Bukti Fisik</th>
								...
								<tbody>
									<?php foreach ($q3->result() as $row) { ?>
										<tr class="text-center">
											...
											<td>Jumlah penulis <?= $row->jml_penulis ?> / Penulis ke-<?= $row->penulis_ke ?> / Koresponden <?= $row->koresponden ?></td>
											<td><?= $row->syarat ?></td>
											...
											<td>
												...
												<?php if ($usulan_status_id <> '1' && $usulan_status_id <> '2') {
													...
													<a href="<?= base_url() ?>usulan/usulan_dupak_b/hapus_B0004/<?= $row->no ?>/<?= $row->usulan_no ?>/<?= $row->berkas ?>/B0026" class="btn btn-danger tombol-hapus">
		...
		<div class="modal-content">
			...
			<form method="post" action="<?= base_url() ?>usulan/usulan_dupak_b/tambah_B0004/<?= $dupak_no ?>/<?= $kum ?>" role="form" enctype="multipart/form-data">
				<div class="modal-body">
					<h3>Bidang B</h3>
					<b>Menghasilkan karya ilmiah hasil penelitian atau pemikiran yang dipublikasikan</b><br>
					Jurnal internasional terindeks pada basis data internasional di luar kategori 2
					<hr>
							...
							<div class="form-group">
								...
								<select name="thn_akademik" class="form-control select2" style="width: 100%;" id="thn_akademik" data-placeholder="Click to Choose..." required>
									...
									<?php if ($jabatan_no <> '31') {
										$thn_jab = substr($jabatan_tgl, 0, 4) - 5;
										$thn_now = date('Y');
										$thn = $thn_now - $thn_jab; ?>
										<option value="<?= $thn_jab_mulai ?>"><?= $thn_jab_mulai ?></option>
										<?php
										for ($i = 0; $i <= $thn; $i++) {
										?>
											<option value="<?= $thn_jab ?>"><?= $thn_jab ?></option>
										<?php
											$thn_jab++;
										}
									} else {
										$thn_jab = substr($pengangkatan_tgl, 0, 4) - 5;
										$thn_now = date('Y');
										$thn = $thn_now - $thn_jab; ?>
										<option value="<?= $thn_jab_mulai ?>"><?= $thn_jab_mulai ?></option>
										<?php
					...
					<div class="row">
						...
						<div class="col-md-4">
							<div class="form-group">
								<label for="k">Sebagai Koresponden</label>
								<select name="k" class="form-control select2" style="width: 100%;" id="k" data-placeholder="Click to Choose..." required>
									<option value=""></option>
					...
					<div class="row">
						...
						<div class="col-md-4">
							<div class="form-group">
								<label>Dijadikan syarat khusus</label>
								<select name="syarat" class="form-control select2" style="width: 100%;" data-placeholder="Click to Choose..." required>
									<option value=""></option>
									<option value="Ya">Ya</option>
									<option value="Tidak">Tidak</option>
								</select>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								...
								<input type="hidden" name="kum" value="40">
								<input type="hidden" name="volum" value="1">
							</div>
						</div>
					</div>
					<div class="row">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label for="url">Alamat Web Jurnal</label>
								<div class="input-group date">
									<input type="text" name="url_web" class="form-control" placeholder="https://ojs.com/Jurnal.pdf" required>
								</div>
								<code>*Mulai tahun 2012, alamat ini harus mengarah ke web jurnal resmi, bukan sekedar repository. Sebaiknya, alamat ini langsung menuju ke artikel, bukan hanya halaman depan (homepage)</code>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label for="url">URL Dokumen (Opsional)</label>
								<div class="input-group date">
									<input type="text" name="url" class="form-control" placeholder="https://repository.com/Jurnal.pdf">
								</div>
								<code>*Alamat menuju repository artikel ini (jika tidak open access) yang meliputi : sampul jurnal, informasi dewan redaksi/editor, daftar isi, dan artikel</code>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label for="url">URL Dokumen Cek Similarity atau Originality</label>
								<div class="input-group date">
									<input type="text" name="url_index" class="form-control" placeholder="https://repository.com/Jurnal.pdf" required>
								</div>
								<code>*URL menuju dokumen hasil pengecekan Similarity atau originality.</code>
							</div>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="text" name="thn_akademik" value="<?= $row->semester ?> <?= $row->tahun_akademik ?>/<?= $row->thn_akademik_baru ?>" class="form-control" id="thn_akademik" maxlength="150" readonly>
								</div>
							</div>
							<div class="col-md-2">
								<div class="form-group">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label for="k">Sebagai Koresponden</label>
									<select name="k" class="form-control select2" style="width: 100%;" id="k" data-placeholder="Click to Choose..." required>
										<option value="<?= $row->koresponden ?>"><?= $row->koresponden ?></option>
										<option value="Ya">Ya</option>
										<option value="Tidak">Tidak</option>
									</select>
								</div>
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label>Dijadikan syarat khusus</label>
									<select name="syarat" class="form-control select2" style="width: 100%;" data-placeholder="Click to Choose..." required>
										<option value="<?= $row->syarat ?>"><?= $row->syarat ?></option>
										<option value="Ya">Ya</option>
										<option value="Tidak">Tidak</option>
									</select>
								</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="url">Alamat Web Jurnal</label>
									<div class="input-group date">
										<input type="text" name="url_web" value="<?= $row->url_web ?>" class="form-control" placeholder="https://ojs.com/Jurnal.pdf" required>
									</div>
									<code>*Mulai tahun 2012, alamat ini harus mengarah ke web jurnal resmi, bukan sekedar repository. Sebaiknya, alamat ini langsung menuju ke artikel, bukan hanya halaman depan (homepage)</code>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="url">URL Dokumen (Opsional)</label>
									<div class="input-group date">
										<input type="text" name="url" value="<?= $row->url ?>" class="form-control" placeholder="https://repository.com/Jurnal.pdf">
									</div>
									<code>*Alamat menuju repository artikel ini (jika tidak open access) yang meliputi : sampul jurnal, informasi dewan redaksi/editor, daftar isi, dan artikel</code>
								</div>
							</div>
						...
						<!-- <div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="url">URL Peer Review</label>
									<div class="input-group date">
										<input type="text" name="url_peer" value="<?= $row->url_peer ?>" class="form-control" placeholder="https://repository.com/Jurnal.pdf">
									</div>
									<code>*URL menuju dokumen peer review (direct link).</code>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="url">URL Dokumen Cek Similarity atau Originality</label>
									<div class="input-group date">
										<input type="text" name="url_index" value="<?= $row->url_index ?>" class="form-control" placeholder="https://repository.com/Jurnal.pdf" required>
									</div>
									<code>*URL menuju dokumen hasil pengecekan Similarity atau originality.</code>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="url">URL Dokumen Bukti Review Artikel (Optional)</label>
									<div class="input-group date">
										<input type="text" name="url_review" value="<?= $row->url_review ?>" class="form-control" placeholder="https://repository.com/Jurnal.pdf">
									</div>
									<code>*URL menuju dokumen bukti review karya ilmiah (apabila diminta oleh penilai).</code>
								</div>
							</div>
