						...
						<div class="card">
							...
							<div class="card-body">
										...
										<tbody>
											...
											<tr>
												...
												<?php if (!empty($row_dos['gelar_depan'])) { ?>
													<td colspan="4" style="vertical-align: top;"><?= $g_depan ?>. <?= $row_dos['nama'] ?>, <?= $row_dos['gelar_belakang'] ?></td>
												<?php } else { ?>
											...
											<tr>
												...
												<td colspan="4" style="vertical-align: top;"><?= $nip ?> / <?= $karpeg ?></td>
											</tr>

											<tr>
												<td colspan="3" style="vertical-align: top;">5. Tempat dan Tanggal Lahir</td>
												<td colspan="4" style="vertical-align: top;"><?= $row_dos['lahir_tempat'] ?>, <?= $tgl ?></td>
											</tr>
											...
											<tr>
												...
												<td colspan="4" style="vertical-align: top;"><?= $jkel ?></td>
											</tr>
											...
											<tr>
												...
												<td colspan="4" style="vertical-align: top;">
													...
													$resume_didik = $this->db->query("SELECT
																						...
																						WHERE d.`no_dosen` <> ''
																							AND a.id_sdm = '$row_dos[dosen_no]'")->result();
													?>
													<?php
											...
											<tr>
												...
												<?php if ($resume_golongan->golongan_kode = '99' || empty($resume_golongan->golongan_kode)) { ?>
													<td colspan="4"><?= $resume_golongan->nama ?> <?= $resume_golongan->kode_gol ?> / <?= $golongan_tgl ?> </td>
												<?php } else { ?>
													<td colspan="4"><?= $resume_golongan->nama ?> <?= $resume_golongan->kode_gol ?> / <?= $golongan_tgl ?></td>
												<?php } ?>
											</tr>
											...
											<tr>
												...
												<?php } elseif ($resume_jabatan->jabatan_no == '31') { ?>
													<td colspan="4"><?= $resume_jabatan->nama_jabatans ?>, <?= $resume_jabatan->kum ?> KUM / <?= $pengangkatan_tgl ?></td>
												<?php } else { ?>
													<td colspan="4"><?= $resume_jabatan->nama_jabatans ?>, <?= $resume_jabatan->kum ?> KUM / <?= $jabatan_tgl ?></td>
												<?php  } ?>
											</tr>
											...
											<tr>
												...
												if (substr($row_dos['instansi_kode'], 0, 3) == '031' || substr($row_dos['instansi_kode'], 0, 3) == '032') { ?>
													...
													<td colspan="4"><?= $row_dos['fakultas'] ?> / <?= $row_dos['nama_prodi'] ?> <?= $jenjang_prodi->nama_jenjang ?></td>
												<?php } else {  ?>
													<td colspan="3">10. Program Studi</td>
													<td colspan="4"> <?= $row_dos['nama_prodi'] ?> <?= $jenjang_prodi->nama_jenjang ?></td>
												<?php } ?>
											</tr>
											...
											<tr>
												...
												<td colspan="2"><?= $rdq_cari_gol['nama'] ?> / <?= $rdq_cari_gol['kode_gol'] ?>/ <?= $rdq_cari_gol['golongan_tgl'] ?></td>
												<td colspan="3"></td>
											</tr>
											<?php
											...
											<tr>
												...
												<?php } elseif ($rdq_cari_jab['jabatan_no'] == '31') { ?>
													<td colspan="2"><?= $rdq_cari_jab['nama_jabatans'] ?> (<?= $rdq_cari_jab['nama_jenjang'] ?>), <?= $rdq_cari_jab['pengangkatan_tgl'] ?></td>
												<?php } else { ?>
								...
								if ($data_dasar->usulan_status_id == '10') {
									if ($role <> '3') {
										// Tutup sementara sampai tgl 17 juli 2022
										if ($todayis < "2022-07-18") {
								?>
											<center>
												<div class="alert alert-danger" role="alert">Proses ajuan usulan baru ditutup sementara dan akan dibuka kembali pada tanggal <b>18 Juli 2022</b>.</div>
											</center>
										<?php
										} elseif ($todayis > $tanggal_buka || $todayis >= $tutup) {
										?>
											<center>
									...
									<?php } elseif ($cari_berkas_c > "0") { ?>
										<center>
											<hr>
											<a href='' data-toggle='modal' id='myLargeModalLabel' data-target='#bid_c' class='btn btn-danger'><i class='fa fa-warning'></i> Pengajuan Usulan Baru <i class='fa fa-warning'></i></a>
											<hr>
										</center>
									<?php } else { ?>
										<center>
		...
		<div class="modal-content">
			...
			<form method="post" action="<?= base_url(); ?>usulan/usulan/proses_usulanbaru/<?php echo $data_dasar->no; ?>/<?= $data_dasar->dosen_no ?>/<?= $jabatan_no; ?>" role="form" enctype="multipart/form-data">
				<div class="modal-body">
					...
					<div class="card">
						<div class="card-body">
							<h4 class="card-title">Mohon Menjadi Perhatian :</h4>
							<ul>
								<li>
									<b>Data pada laman ini adalah data yang akan digunakan pada dokumen PAK dan SK Jabatan Akademik, mohon dipastikan bahwa data Bapak/Ibu sudah benar.</b>
								</li><br>
								<?php if ($usulan_status_id == "1") { ?>
									<li>
										<b>Usulan Bapak/Ibu akan diajukan dahulu ke Admin Perguruan Tinggi untuk di verikasi dan diupload kelengkapan dokumen persyaratan</b>
									</li><br>
								<?php } ?>

								<?php if ($usulan_status_id == "10") { ?>
									<li>
										<b>Usulan akan diajukan ke LLDIKTI III untuk di verifikasi kelengkapan dokumen persyaratan</b>
									</li><br>
								<?php } ?>
								<li>
									<b>Jika Anda yakin data Anda sudah benar, Silakan klik tombol Kirim.</b>
								</li>
							</ul>
						</div>
					</div>
					...
					<?php if ($data_dasar->usulan_status_id == '1') { ?>
						...
						<input type="hidden" name="usulan_ket" value="">
...
<!-- Modal warning bidang C-->
<div class="modal fade" aria-labelledby="myLargeModalLabel1" id="bid_c">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
			</div>
			<div class="modal-body">
				<div style="text-align: center; color: #8c0000; font-size: 2.3em; padding-bottom: 10px"><strong>Peringatan..!!!</strong></div>
				<div style="text-align: center; color: #000066; font-size: 1.5em;">Masih terdapat bukti fisik pada Bidang C yang belum Anda lengkapi. Silakan cek kembali pada Bidang C.<br><br>
				</div>
			</div>
			<div class="modal-footer">
				<div class="btn-group pull-right">
