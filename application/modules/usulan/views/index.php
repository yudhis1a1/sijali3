                ...
                <div class="navbar-header">
                    <a class="navbar-brand" href="<?php echo base_url(); ?>usulan">
                        <!-- Logo icon --><b>
                            ...
                            <!-- Dark Logo icon -->
                            <img src="<?= base_url() ?>assets/images/logo-lldikti-icon.png" alt="Sijali-3" class="dark-logo" />
                            <!-- Light Logo icon -->
                            <img src="<?= base_url() ?>assets/images/logo-lldikti-icon.png" alt="Sijali-3" class="light-logo" />
                        </b>
                        <!--End Logo icon -->
                        <!-- Logo text --><span>
                            <!-- dark Logo text -->
                            <img src="<?= base_url() ?>assets/images/sijali_hebat.png" alt="Sijali-3" class="dark-logo" height="55" width="165" />
                            <!-- Light Logo text -->
                            <img src="<?= base_url() ?>assets/images/sijali_hebat.png" class="light-logo" alt="Sijali-3" height="55" width="165" />
                        </span>
                    </a>
                </div>
                <!-- ============================================================== -->
...
<script>
    $(document).ready(function() {
        $("#myTablesya").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "<?php echo base_url('usulan/usulan/usulan/' . $filter) ?>",
                type: 'POST',
            }
        });
        ...
        $("#tableDosen").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "<?php echo base_url('ketenagaan/ketenagaan/tampil_data_dosen_all') ?>",
                type: 'POST',
            }
        });
        ...
        $('#myTablesya_pimpinan').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "<?php echo base_url('usulan/pimpinan/usulan_pimpinan') ?>",
                type: 'POST',
            }
        });
...
<!-- <script>
    $(document).ready(function() {
        $('select[name="semester"]').change(function() {
            ...
            $.ajax({
                type: 'post',
                url: '<?= base_url() ?>usulan/usulan_dupak/get_mata_kuliah',
                data: {
                    semester: semester
                },
                success: function(response) {
                    var mataKuliah = JSON.parse(response);
                    var options = '';
                    for (var i = 0; i < mataKuliah.length; i++) {
                        options += '<option value="' + mataKuliah[i].NMMK + '">' + mataKuliah[i].NMMK + '</option>';
                    }
                    $('select[name="uraian[]"]').html(options);
                }
            });
        });
    });
        ...
        $('select[name="semester"]').change(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (var i = 0; i < mataKuliah.length; i++) {
                        var nosul = usulan_no_details + i;
                        rows += '<tr>' +
                            '<td><input type="hidden" name="no_usulan_detail[]" value="' + nosul + '">' + (i + 1) + '</td>' +
                            '<td> <input type="hidden" value="' + mataKuliah[i].NMMK + '" name="uraian[]">' + mataKuliah[i].NMMK + '</td>' +
                            '<td class="text-center"><input type="text" placeholder="..." style="width:100px;" value="' + mataKuliah[i].SKS + '" name="sks[]" class="form-control" required></td>' +
                            '<td class="text-center"><input type="number" min="0" placeholder="..." style="width:100px;" name="jumlah_volume[]" class="form-control" required></td>' +
                            '</tr>';
                    }
                    $('#mata-kuliah').html(rows);
                }
            });
        });
    });
...
<script>
    $(document).ready(function() { // Ketika halaman sudah diload dan siap
        $("#btn-tambah-form").click(function() { // Ketika tombol Tambah Data Form di klik
            ...
            var q1 = "<?php foreach ($ajar_dosen as $ajar) { ?>";
            var q2 = "<select name='uraian[]' class='form-control select2' style='width: 450px' data-placeholder='Pilih Mata Kuliah..' required><?php foreach ($ajar_dosen as $ajar) { ?> <option value='<?php echo preg_replace("/[^a-zA-Z0-9 ]/", "", $ajar->NMMK); ?>'><?php echo preg_replace("/[^a-zA-Z0-9 ]/", "", $ajar->NMMK); ?></option> <?php } ?> </select>";
            ...
            $("#insert-form").append("<h4>Data Mata Kuliah ke " + nextform + " :</h4>" +
                ...
                "<tr>" +
                "<td>Matakuliah</td>" +
                "<td>" + q2 + " <input type='hidden' name='no_usulan_detail[]' value='" + next_usulan + "'> </td>" +
                "</tr>" +
                "<tr>" +
                "<td>SKS/mtk</td>" +
                "<td><input type='text' name='sks[]' style='width:70px;' class='form-control'  required>&nbsp;&nbsp;&nbsp;" +
                "Jumlah Kelas : <input type='text'  name='jumlah_volume[]'class='form-control'  style='width:70px;' required><br>" +
    ...
    $(document).ready(function() {
        ...
        $('#sample_form').on('submit', function(event) {
            ...
            if (count_error == 0) {
                $.ajax({
                    ...
                    url: "<?php echo base_url('usulan/usulan/update_miror/' . $nidn_dos) ?>",
                    method: "POST",
                    data: $(this).serialize(),
                    beforeSend: function() {
        ...
        function progress_bar_process(percentage, timer) {
            ...
            if (percentage > 100) {
                ...
                $('#success_message').html("<div class='alert alert-success'>Data Berhasil Disinkronkan, Silakan Refresh untuk melihat perubahan data Anda. </div>");
                setTimeout(function() {
...
<script>
    $(document).ready(function() {
        ...
        $('#form_pend').on('submit', function(event) {
            event.preventDefault();
            var count_error = 0;
            ...
            if (count_error == 0) {
                $.ajax({
                    ...
                    url: "<?php echo base_url('usulan/usulan/update_rwy_pend/' . $nidn_dos) ?>",
                    method: "POST",
                    data: $(this).serialize(),
                    beforeSend: function() {
                        $('#save_pend').attr('disabled', 'disabled');
                        $('#process_pend').css('display', 'block');
                    },
                    success: function(data) {
                        var percentage = 0;
                        ...
                        var timer = setInterval(function() {
                            percentage = percentage + 20;
                            progress_bar_process_pend(percentage, timer);
                        }, 1000);
                    }
                })
            } else {
        ...
        function progress_bar_process_pend(percentage, timer) {
            $('.progress-bar').css('width', percentage + '%');
            $('.progress-bar').html(percentage + '%');
            if (percentage > 100) {
                clearInterval(timer);
                $('#form_pend')[0].reset();
                $('#process_pend').css('display', 'none');
                $('.progress-bar').css('width', '15%');
                $('#save_pend').attr('disabled', false);
                $('#success_message_pend').html("<div class='alert alert-success'>Data Berhasil Disinkronkan, Silakan Refresh untuk melihat perubahan data Anda. </div>");
                setTimeout(function() {
                    $('#success_message_pend').html('');
                    location.reload()
                }, 10);
            }
        }
...
<script>
    $(document).ready(function() {
        ...
        $('#form_ajar').on('submit', function(event) {
            event.preventDefault();
            var count_error = 0;
            ...
            if (count_error == 0) {
                $.ajax({
                    ...
                    url: "<?php echo base_url('usulan/usulan/update_ajar_miror') ?>",
                    method: "POST",
                    data: $(this).serialize(),
                    beforeSend: function() {
                        $('#save_ajar').attr('disabled', 'disabled');
                        $('#process_ajar').css('display', 'block');
                    },
                    success: function(data) {
                        var percentage = 0;
                        ...
                        var timer = setInterval(function() {
                            percentage = percentage + 20;
                            progress_bar_process_ajar(percentage, timer);
                        }, 1000);
                    }
                })
            } else {
        ...
        function progress_bar_process_ajar(percentage, timer) {
            $('.progress-bar').css('width', percentage + '%');
            $('.progress-bar').html(percentage + '%');
            if (percentage > 100) {
                clearInterval(timer);
                $('#form_ajar')[0].reset();
                $('#process_ajar').css('display', 'none');
                $('.progress-bar').css('width', '15%');
                $('#save_ajar').attr('disabled', false);
                $('#success_message_ajar').html("<div class='alert alert-success'>Data Berhasil Disinkronkan, Silakan Refresh untuk melihat perubahan data Anda. </div>");
                setTimeout(function() {
                    $('#success_message_ajar').html('');
                    location.reload()
                }, 10);
            }
        }
...
<script>
    $(document).ready(function() {
        $("#dosen").autocomplete({
            source: "<?php echo site_url('akun/akun/cari_dosen_pt'); ?>"
        });
    });
</script>
