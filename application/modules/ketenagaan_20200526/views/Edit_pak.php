...
?>       
              ...
              <td colspan="5"><?=$resume_jabatan->nama_jabatans?>, <?=$resume_jabatan->kum?> KUM / <?=$jabatan_tgl?></td>  
                    ...
                    <tr>
                      ...
                      <td colspan="5"><input type="text" name="fakultas"  value="<?=$resume->fakultas?>" style="width: 250px;" minlength="5" required="required"> / <?=$resume->nama_prodi?> <?=$jenjang_prodi->nama_jenjang?></td>
                      <?php } else {  ?>
                    ...
                    <?php
                     if($resume->jabatan_no == '31') {
                      $diff_lama  = date_diff( date_create(	$resume_jabatan->pengangkatan_tgl), date_create($resume_jabatan->pengangkatan_tgl) );
                     // $diff_baru  = date_diff( date_create(	$resume_jabatan->pengangkatan_tgl), date_create($resume->tmt_tgl) );
                      $diff_baru  = date_diff( date_create(	$resume_jabatan->pengangkatan_tgl), date_create($tgltmt) );
                    } else {
                    ...
                    <tr>
                      ...
                      <td colspan="5"><?=$diff_baru->format('%y')?> Tahun <?=$diff_baru->m+1?> Bulan</td>
                    </tr>
...
?>
             ...
             /*  $kum_a=69.50;
              ...
              $proposional_a=$lebihan_a /($lebihan_a+$lebihan_b)*(($kebutuhan_c - $kat_4->kum_penilai_baru)
              +($kebutuhan_d - $kat_5->kum_penilai_baru));
             
              $proposional_b=$lebihan_b /($lebihan_a+$lebihan_b)*(($kebutuhan_c- $kat_4->kum_penilai_baru)
              +($kebutuhan_d - $kat_5->kum_penilai_baru));
           ...
           ?>
					...
					$qajar=$this->db->query("SELECT * from usulan_matkuls where usulan_no='$no'");
                      ...
                      <?php if(($resume_jabatan->jabatan_no=='31') || empty($resume_jabatan->jabatan_no)){  ?>
                       ...
                       <br>
                        Dalam Mata Kuliah :
                        <?php } else { ?>
                          Dapat Dinaikan Dalam Jabatan Akademik Dosen :  <b style="font-weight: bold;"><?=$resume_jab_usulan->nama_jabatans?> <?=$resume_jab_usulan->kum?> KUM</b>, Terhitung Mulai Tanggal <b style="font-weight: bold;"><?=$tgl_terhitung;?></b>
            ...
            <div class="table-responsive">
                <table width="100%">
                    <tr>
                      <td colspan="5" style="vertical-align: top; border-left: none; border-right: none; width: 60%; font-weight: bold; font-family: Calibri; font-size:16px;">
                      <?php if(!empty($resume->gelar_depan)){ ?>
