                ...
                <div class="col-10">
                    <select class="select2 m-b-10 " style="width: 100%" onchange="changeValue(this.value)">
                        ...
                        foreach ($dosen->result() as $row) {
                            echo '<option value="' . $row->nidn . '">(' . $row->nidn . ') - ' . $row->nama . ' - ' . $row->nama_jabatans . ' ( ' . $row->kum . ' )</option>';

                            $jsArray .= "prdName['" . addslashes($row->nidn) . "'] = {nidn:'" . addslashes($row->nidn) . "',nama:'" . addslashes($row->nama) . "'};\n";
                        }
                        ?>
                    </select>
