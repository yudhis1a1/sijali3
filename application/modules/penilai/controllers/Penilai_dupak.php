...
class Penilai_dupak extends MX_Controller
{
	...
	berkas  FROM  `usulan_dupak_details` WHERE usulan_no = '$id' AND `dupak_no` = '$no'");
		...
		$A0001['q2'] 		= $q2;
		$A0001['q3'] 		= $q3;
		$A0001['dosen_no'] 	= $data_dasar->dosen_no;
		$A0001['usulan'] 	= $q_usulan;

		if ($no == 'A0001') {
			if ($q1->id_rwy_didik_formal == '') {
				vpenilai('bidang_a/a0001_lama', $A0001);
			} else {
				vpenilai('bidang_a/a0001', $A0001);
			}
		} elseif ($no == 'A0002') {
			if ($q1->id_rwy_didik_formal == '') {
				vpenilai('bidang_a/a0002_lama', $A0001);
			} else {
	...
	function update_mengajar($dupak)
	{
	...
	function update_ijazah($dupak)
	{
		$no_dupak_details 		= $this->input->post('no');
		$no_usulan 				= $this->input->post('no_usulan');
		$kum_dosen 				= $this->input->post('kum_dosen');
		$user_penilai_no 		= $this->input->post('user_penilai_no');
		$kum_penilai_keterangan = addslashes($this->input->post('kum_penilai_keterangan'));
		$cek_nilai 				= $this->input->post('cek_nilai');
		$tgl_update 			= date("y-m-d H:i:s");
		...
		if ($cek_nilai == '') {
			$this->db->query("UPDATE
								usulan_dupak_details
							SET
								kum_penilai 	= '$kum_dosen',
								status_ijazah 	= 'diterima'
							WHERE usulan_no='$no_usulan'
							AND dupak_no='$dupak'");
		} else { //Jika ceklis ditolak
			$this->db->query("UPDATE
								usulan_dupak_details
							SET
								kum_penilai 	= '0',
		...
		$this->db->query("UPDATE
							...
							SET
								...
								kunci='1',
								user_penilai_no='$user_penilai_no',
								updated_at='$tgl_update'
							WHERE usulan_no='$no_usulan'
								AND dupak_no='$dupak'");
