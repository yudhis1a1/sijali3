...
<div class="row">
	...
	<div class="card-body">
		...
		<form method="post" action="<?= base_url()?>penilai/penilai_dupak_c/update_C0015/<?=$q_dupak->dupak_no?>" role="form" enctype="multipart/form-data">
		<table border="1" class="ui celled table" width="100%">
			<thead>
				<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<?php  if(isset($row->berkas)){ ?>
						<?php if($role<>'3' || $usulan_status_id<>'1' && $usulan_status_id<>'2'){?>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/download_bidangc/<?=$row->berkas?>" target="blank" class="btn btn-xs btn-success" ><i class="fa  fa-cloud-download"></i>[Surat Tugas]</a>
						<?php }else{ ?>
							<a href="<?= base_url()?>usulan/usulan_dupak_c/download_bidangc/<?=$row->berkas?>" target="blank" class="btn btn-xs btn-success" ><i class="fa  fa-cloud-download"></i>[Surat Tugas]</a>
				...
				<tr style="background-color: <?=$warna?>;  font-weight: bold;">
                  <td>
                    FORM PENILAIAN:
                    <br><i>Dilakukan penilaian oleh <?=$r_penilai['nama']?><input type="hidden" name="user_penilai_no[]" value=<?=$r_penilai['user_penilai_no']?>><input type="hidden" name="no[]" value=<?=$row->no?>><input type="hidden" name="sms[]" value="semester,<?=$row->tahun_akademik?>"><input type="hidden" name="no_usulan" value=<?=$q1->usulan_no?>>
				  </td>
                  <td colspan="4">
                    KETERANGAN:
