		...
		<table border="1" class="ui celled table" width="100%">
			...
			<tbody>
				...
				<tr style="background-color: <?=$warna?>;  font-weight: bold;">
                  <td>
                    FORM PENILAIAN:
                    <br><i>Dilakukan penilaian oleh <?=$r_penilai['nama']?></i><input type="hidden" name="user_penilai_no[]" value="<?=$r_penilai['user_penilai_no']?>"><input type="hidden" name="no[]" value=<?=$row->no?>><input type="hidden" name="sms[]" value="<?=$row->semester?>,<?=$row->tahun_akademik?>"><input type="hidden" name="no_usulan" value=<?=$q1->usulan_no?>>
				  </td>
                  <td colspan="4">
                    KETERANGAN:
