...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
  		    <h3>BIDANG D</h3>
			<b>Menjadi anggota dalam suatu Panitia/Badan  pada Perguruan Tinggi</b><br>
...
<div class="row">
	...
	<div class="card-body">
		...
		<form method="post" action="<?= base_url()?>penilai/penilai_dupak_d/update_D0001/<?=$q_dupak->dupak_no?>" role="form" enctype="multipart/form-data">
		<table border="1" class="ui celled table" width="100%">
			<thead>
				<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
			...
			<tbody>
				...
				<tr style="background-color: <?=$warna?>; font-weight: bold;">
                  <td>
                    FORM PENILAIAN:
                    <br><i>Dilakukan penilaian oleh <?=$r_penilai['nama']?><input type="hidden" name="user_penilai_no[]" value=<?=$r_penilai['user_penilai_no']?>><input type="hidden" name="no[]" value=<?=$row->no?>><input type="hidden" name="thn_akademik[]" value="<?=$row->tahun_akademik?>"><input type="hidden" name="no_usulan" value=<?=$q1->usulan_no?>>
				  </td>
                  <td colspan="4">
                    KETERANGAN:
                  ...
                  <td style="min-width: 50px">
				    PENILAIAN<br>TIM PENILAI:<br><select name="kum_penilai[]"><option value="" disabled selected hidden>Pilih Nilai AK</option><?php for($i=0;$i<=$kum;$i+=0.5){ ?><option value="<?=$i?>,<?=$row->tahun_akademik?>"><?=$i?></option><?php };?></select>
                  </td>
                 </tr>
				<?php
				}
