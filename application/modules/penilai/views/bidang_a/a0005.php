...
<div class="row">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<tbody>
					...
					<tr class="text-center">
						...
						<td></td>
						<td>
							<?php if($usulan_status_id<>'5'){}else{ ?><a href="" data-toggle="modal" id="myLargeModalLabel" data-target="#edit"  class="btn btn-warning"><i class="fa fa-edit"> EDIT</i></a><?php } ?>
						</td>
						<td></td>
						<td></td>
					</tr>
	...
	<div class="card-body">
		...
		<form method="post" action="<?= base_url()?>penilai/penilai_dupak/update_A0001/A0005" role="form" enctype="multipart/form-data">
		<table border="1" class="ui celled table" width="100%">
			<thead>
				<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<td><?=$row->keterangan?><br><a href="<?= base_url()?>penilai/penilai_dupak/download_bidanga/<?=$row->berkas?>" target="blank" class="btn  btn-success" ><i class="fa  fa-cloud-download"></i>[PDF]</a>
				</tr>
				<?php
			...
			<tr class="text-center"><?php if($usulan_status_id<>'5'){}else{ ?><center><a href="<?=base_url()?>penilai/penilai_dupak/hapus_a0004/<?=$q_dupak->dupak_no?>/<?=$q_dupak->usulan_no?>" class="btn btn-danger">RESET PENILAIAN</a>&nbsp;&nbsp;&nbsp;<button type="submit" class="btn btn-success">SIMPAN PENILAIAN</button></center><?php } ?></tr>
		</form>
	</div>
	<?php
	}
