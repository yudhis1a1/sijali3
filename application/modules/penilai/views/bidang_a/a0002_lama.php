		...
		<div class="card">
			...
			?>
				<div class="card-body">
					...
					<form method="post" action="<?= base_url() ?>penilai/penilai_dupak/update_ijazah/A0002" role="form" enctype="multipart/form-data">
						<table border="1" class="ui celled table" width="100%">
							<thead>
								<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
							...
							<tbody>
								...
								<tr class="text-center">
									...
									<td><a href="<?= $rp->link_tesis ?>" target="_blank"><?= $rp->link_tesis ?></a></td>
									<td>
										Scan Ijazah: <br>
										<?php
										if (isset($q1->berkas)) { ?>
										...
										?>
										<br>
										File Resume Tesis: <br>
										<?php
										if ($rp->tesis <> '') { ?>
											<a href="<?= base_url() ?>usulan/usulan/download_file_rwy_pendidik/<?= $rp->tesis ?>" target="blank" class="btn btn-xs btn-success"><i class="fa  fa-cloud-download"></i>[File Tesis]</a>
										<?php
										} else { ?>
											<a href="#" class="btn btn-xs btn-danger"><i class="fa fa-ban"></i>[File Tesis]</a><br>
										<?php
										}
										?>
										<br>
