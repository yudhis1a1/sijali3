		...
		<div class="card">
			...
			<?php if ($q2->row() > 0) { ?>
				<div class="card-body">
					...
					<form method="post" action="<?= base_url() ?>penilai/penilai_dupak/update_mengajar/A0004" role="form" enctype="multipart/form-data">
								...
								<table border="1" class="table table-bordered" width="100%">
									<thead>
										...
										<tr style="background-color: #e4e4e4;">
											<th colspan="2" class="text-center">
												<?php
												$cd = $this->db->query("SELECT
																			LEFT(`created_at`,10) AS tgl_buat
																		FROM
																			`usulan_dupak_details`
																		WHERE `dupak_no` = 'A0004'
																			AND `usulan_no` = '$no'
																			AND LEFT(`created_at`,10)>'2022-12-21'
												...
												?>
													...
													?>
												<?php } else { ?>
													Bukti Fisik : <a href="<?= base_url() ?>penilai/penilai_dupak/download_bidanga/<?= $row_thn['berkas'] ?>" target="blank" class="btn  btn-success"><i class="fa  fa-cloud-download"></i>[PDF]</a>
												<?php } ?>
											</th>
											<?php
											$total_sks = 0;
											...
											?>
											<th colspan="2" class="text-center" style="background-color: #e4e4e4; font-weight: bold; font-size:15px;">
												Angka Kredit : <?= $ak ?>
											</th>
										</tr>
										<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
									...
									<tbody>
										...
										<tr style="background-color: <?= $warna ?>; font-weight: bold;">
											...
											<td colspan="2">
												KETERANGAN:
												<textarea name="kum_penilai_keterangan[]" class="form-control" rows="5"><?= $row_thn['kum_penilai_keterangan'] ?></textarea>
											</td>
											<td style="min-width: 50px">
												PENILAIAN<br>TIM PENILAI:<br>
												<input type="number" step="any" name="kum_penilai[]" class="form-control" value="<?= $row_thn['kum_penilai'] ?>" min="0" required><br>
