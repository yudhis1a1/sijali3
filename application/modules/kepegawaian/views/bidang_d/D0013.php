...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
  		    <h3>Bidang D</h3>
			<b>Mewakili perguruan tinggi/lembaga pemerintah</b><br>
			Mewakili perguruan tinggi/ Iembaga pemerintah duduk dalam panitia antar lembaga
		</div>
    </div>
	</div>
	</div>
	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<tbody>
				  ...
				  <input type="hidden" class="form-control" value="D0013" name="bidang" readonly>
		...
		<table border="1" class="ui celled table" width="100%">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<td><?=$row->keterangan?><br><a href="<?= base_url()?>kepegawaian/usulan_dupak_d/download_bidangd/<?=$row->berkas?>" class="btn  btn-success" ><i class="fa  fa-cloud-download"></i>[PDF]</a></td>
					<td>
					<?php if($usulan_status_id<>'1' && $usulan_status_id<>'2'){ }else{ ?>
					<a href="<?= base_url()?>kepegawaian/usulan_dupak_d/hapus_D0003/<?=$row->no?>/<?=$row->usulan_no?>/<?=$row->berkas?>/D0013/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>kepegawaian/usulan_dupak_d/tambah_D0003/D0013/<?= $kum ?>" role="form" enctype="multipart/form-data">
