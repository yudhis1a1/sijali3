...
<div class="row">
	...
	<div class="card-body">
	  	<div class="tile">
  		    <h3>Bidang B</h3>
			<b>Membuat rancangan dan karya teknologi yang tidak dipatenkan; rancangan dan karya seni monumental yang tidak terdaftar di HaKI tetapi telah dipresentasikan pada forum yang teragenda</b><br>
			Tingkat nasional 	
		</div>
    </div>
	</div>
	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<?php if($usulan_status_id=='3'){ ?>
				  ...
				  <input type="hidden" class="form-control" value="B0022" name="bidang" readonly>
		...
		<table border="1" class="ui celled table" width="100%">
			...
			<?php foreach($q3->result() as $row){?>
				...
				<td><?=$row->uraian?></td>
					...
					<td><?=$row->tgl?></td>
					<td>
						Alamat Website/URL Dokumen : <a href="<?=$row->url?>" target="_blank"><?=$row->url?></a><br>
					...
					<a href="<?= base_url()?>kepegawaian/usulan_dupak_b/hapus_B0005/<?=$row->no?>/<?=$row->usulan_no?>/<?=$row->berkas?>/B0022/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>kepegawaian/usulan_dupak_b/tambah_B0005/B0022/<?= $kum ?>" role="form" enctype="multipart/form-data">
