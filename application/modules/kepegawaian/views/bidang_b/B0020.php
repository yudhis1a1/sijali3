	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<?php if($usulan_status_id=='3'){ ?>
				  ...
				  <input type="hidden" class="form-control" value="B0020" name="bidang" readonly>
			...
			<?php foreach($q3->result() as $row){?>
				...
				<td><?=$row->uraian?></td>
					...
					<td>
						...
						Alamat Website : <a href="<?=$row->url_web?>" target="_blank"><?=$row->url_web?></a><br>
					</td>
					...
					<a href="<?= base_url()?>kepegawaian/usulan_dupak_b/hapus_B0005/<?=$row->no?>/<?=$row->usulan_no?>/<?=$row->berkas?>/B0020/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>kepegawaian/usulan_dupak_b/tambah_B0005/B0020/<?= $kum ?>" role="form" enctype="multipart/form-data">
