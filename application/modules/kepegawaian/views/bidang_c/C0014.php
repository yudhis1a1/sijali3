	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<tbody>
				  ...
				  <input type="hidden" class="form-control" value="C0014" name="bidang" readonly>
		...
		<table border="1" class="ui celled table" width="100%">
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<td><?=$row->keterangan?><br><a href="<?= base_url()?>kepegawaian/usulan_dupak_c/download_bidangc/<?=$row->berkas?>" class="btn  btn-success" ><i class="fa  fa-cloud-download"></i>[PDF]</a></td>
					<td>
					<?php if($usulan_status_id<>'1' && $usulan_status_id<>'2'){}else{ ?>
					<a href="<?= base_url()?>kepegawaian/usulan_dupak_c/hapus_C0001/<?=$row->no?>/<?=$row->usulan_no?>/<?=$row->berkas?>/C0014/<?=$kum?>" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>kepegawaian/usulan_dupak_c/tambah_C0001/C0014/<?= $kum ?>" role="form" enctype="multipart/form-data">
