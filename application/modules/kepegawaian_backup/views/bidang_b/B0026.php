	...
	<div class="card-body">
        ...
        <table border="1" class="ui celled table" width="100%">
				...
				<?php if($usulan_status_id=='3'){ ?>
				  ...
				  <input type="hidden" class="form-control" value="B0026" name="bidang" readonly>
		...
		<table border="1" class="ui celled table" width="100%">
			<thead>
				<tr style="background-color: #e4e4e4; font-weight: bold;" class="text-center">
					...
					<th>Penulis Ke</th>
					<th>Koresponden</th>
					<th>Penulis Pertama Sekaligus Koresponden</th>
					<th>Angka Kredit</th>
			...
			<?php foreach($q3->result() as $row){?>
				<tr class="text-center">
					...
					<td><?=$row->pertama_sbg_koresponden?></td>
					<td><?=$row->angka_kredit?></td>
					<td><?=$row->keterangan?><br><a href="<?= base_url()?>kepegawaian/usulan_dupak_b/download_bidangb/<?=$row->berkas?>" class="btn  btn-success" ><i class="fa  fa-cloud-download"></i>[PDF]</a></td>
					<td>
					<?php if($usulan_status_id<>'1' && $usulan_status_id<>'2'){}else{ ?>
					<a href="<?= base_url()?>kepegawaian/usulan_dupak_b/hapus_B0004/<?=$row->no?>/<?=$row->usulan_no?>/<?=$row->berkas?>/B0026" class="btn btn-danger tombol-hapus">
...
<div class="modal fade"  aria-labelledby="myLargeModalLabel" id="tambah" >
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="<?= base_url()?>kepegawaian/usulan_dupak_b/tambah_B0004/B0026/<?=$kum?>" role="form" enctype="multipart/form-data">
		<div class="modal-body">
			...
			Jurnal internasional terindeks pada basis data internasional di luar 
				...
				<div class="row">
				  <div class="col-md-6">
                    <div class="form-group">
                      <label for="pdk">Penulis Pertama sekaligus Koresponden</label>
                        <select name="pdk" class="form-control select2"  style="width: 100%;" id="pdk" data-placeholder="Click to Choose..." required>
                          <option value=""></option>
                          <option value="Ya">Ya</option>
                          <option value="Tidak">Tidak</option>
                        </select>
                ...
                <div class="col-md-12">
                <div class="form-group">
                  <label for="uraian">Judul Jurnal</label>
